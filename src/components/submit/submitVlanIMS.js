...
const SubmitVlanIMS = () => {
    ...
    const { data, loading } = useAsync(() => ServiceVlanIMS.getAllVlanIMS())
    const [form] = useForm();
    ...
    const handleEdit = async () => {
        ...
        if (number) {
            const res = await ServiceVlanIMS.editVlanIMS({ number: number }, idEdit)
    ...
    const handleDelete = async (_id) => {
        const res = await ServiceVlanIMS.deleteVlanIMS(_id)
        if (res.detail.msg === "success") {
    ...
    const handleSubmit = async () => {
        const number = form.getFieldValue('number');
        if (number) {
            const res = await ServiceVlanIMS.createVlanIMS({ number: number })
